"use strict";
const screen = document.getElementById("screen"), xmlns = "http://www.w3.org/2000/svg", xlinkns = "http://www.w3.org/1999/xlink";
const devtools = { open: false }, element = new Image();
Object.defineProperty(element, 'id', { get: () => devtools.open = true });
console.log(element);

window.addEventListener("pointermove", (e) => { pointer.x = e.clientX; pointer.y = e.clientY; rad = 0; }, false);
...
const run = () => {
	...
	let e = elems[0];
	const ax = (Math.cos(3 * frm) * rad * width) / height, ay = (Math.sin(4 * frm) * rad * height) / width;
	e.x += (ax + pointer.x - e.x) / 10; e.y += (ay + pointer.y - e.y) / 10;
	for (let i = 1; i < N; i++) {
		let e = elems[i], ep = elems[i - 1], a = Math.atan2(e.y - ep.y, e.x - ep.x);
